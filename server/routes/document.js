...
documentRoutes.route('/create').put(function (req, res, next) {
  ...
  global.entries.addDocument(safePath, req.user.email, req.body.template).then(() => {
    res.status(200).send({
      'path': req.body.path
    })
  }).catch(err => {
...
documentRoutes.route('/edit').post(function (req, res, next) {
  ...
  global.entries.editDocument(safePath, req.body.content, req.user).then(() => {
    res.status(200).send()
  }).catch(err => {
    global.winston.error(err)
    res.status(500).send()
...
documentRoutes.route('/all').post(function (req, res, next) {
  global.entries.getFromTree(req.body.path, req.user).then(results => {
...
documentRoutes.route('/search/:query').get(function (req, res, next) {
  ...
  global.search.searchEntry(query).then(results => {
    res.status(200).send(results)
  })
  // db.Document.search(query, function (err, data) {
...
documentRoutes.route('/*').put(function (req, res, next) {
  ...
  if (!global.rights.checkRole('/documents/' + newPath, res.locals.user.rights, 'write')) {
  ...
  global.entries.moveDocument(oldPath, newPath).then(() => {
    res.status(200).send()
  }).catch(err => {
    global.winston.error(err)
    res.status(500).send()
...
documentRoutes.route('/*').delete(function (req, res, next) {
  ...
  global.entries.deleteDocument(safePath).then(() => {
    res.status(200).send()
  }).catch(err => {
    global.winston.error(err)
    res.status(500).send()
...
documentRoutes.route('/favorites').get(function (req, res, next) {
  ...
  }).select('-user').populate('document').then(favorites => {
  ...
  global.db.Document.findOne({
    ...
    return new Promise(function (resolve, reject) {
      ...
      return global.db.Favorite.findOne({
        ...
        return global.entries.getFromTree(uentry._id, res.locals.user).then(children => {
